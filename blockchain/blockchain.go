...
const (
	dbPath = "./tmp/blocks"
	lhKey  = "lh" // Key for the Last Hash
)
...
func InitBlockChain() *BlockChain {
	...
	// Suppress BadgerDB logging for cleaner output
	opts.Logger = nil
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		if _, err := txn.Get([]byte(lhKey)); err == badger.ErrKeyNotFound {
			...
			// Blockchain exists, read the last hash
...
func (chain *BlockChain) AddBlock(data string) {
	...
	// 3. Save the new block and update the last hash
